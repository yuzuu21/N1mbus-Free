...
void main() {
    ...
    if (hitIdx != -1) {
        ...
        if (alpha > 0.0) {
            ...
            // Deep Midnight Background for Neon
            vec3 panelCol = mix(disp, vec3(0.02, 0.02, 0.05), 0.6);
            panelCol = mix(panelCol, tint.rgb, tint.a * 0.2);
            
            // Neon Tube Glow (The real thing)
            // Core bright line
            float core = exp(-abs(minDist) * 1.5);
            // Outer soft glow
            float glow = exp(-abs(minDist) * 0.2);
            // Flickering light
            float flicker = 0.9 + 0.1 * sin(Time * 40.0 * (1.0 + float(hitIdx) * 0.1));
            
            vec3 neonCol = tint.rgb * (core * 2.0 + glow * 0.8) * flicker;
            panelCol += neonCol;
            
            // Adding a glossy reflection
            float reflection = smoothstep(0.4, 0.5, sin(coord.y * 0.02 - Time * 2.0 + float(hitIdx)));
            panelCol += vec3(1.0) * reflection * 0.03;
